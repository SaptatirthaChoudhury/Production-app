...
const updateUserAvatar = asyncHandler(async (req, res) => {
    ...
    if (!avatar.url) throw new ApiError(400, "Error while uploading on avatar image")
    ...
    const user = await User.aggregate([
        ...
        },
        {
            $lookup: {
                ...
                foreignField: "_id",
                as: "watchHistory",
                pipeline: [                               // Injected Sub-pipeline in videos
                    {
                        $lookup: {
                            from: "users",
                            localField: "owner",
                            foreignField: "_id",
                            as: "owner",
                            pipeline: [                 // Injected Sub-pipeline in users 
                                {
                                    $project: {
                                        fullname: 1,
                                        username: 1,
